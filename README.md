...
- **Users** make realistic edits to comments to avoid toxic classifications :contentReference[oaicite:3]{index=3}
...
- `text`
- `label` :contentReference[oaicite:4]{index=4}

## Methods

### 1. Toxic Comment Classifier
A **Logistic Regression** model is trained using **TF-IDF** vectorized comment text. The data is split into training and test sets before model fitting. :contentReference[oaicite:5]{index=5}

### 2. LIME Explanations
**LIME (Local Interpretable Model-agnostic Explanations)** is used to explain individual predictions by assigning importance weights to words in a comment. Words with higher positive weights push the prediction toward the **toxic** class, while negative weights push it away. :contentReference[oaicite:6]{index=6} :contentReference[oaicite:7]{index=7}
...
- checking whether the prediction flips from **toxic** to **non-toxic** :contentReference[oaicite:8]{index=8} :contentReference[oaicite:9]{index=9}
...
### Faithfulness
Evaluated using a **deletion test**, where removing the most important words should change the model prediction more than removing random words. This is mainly used to assess **LIME**. :contentReference[oaicite:10]{index=10}

### Actionability
Evaluated by whether the counterfactual explanation suggests **small, realistic edits** that flip a prediction from toxic to non-toxic. This is mainly used to assess **counterfactual explanations**. :contentReference[oaicite:11]{index=11}
...
- The classifier relies heavily on **keyword-based signals**, which is expected for a TF-IDF + Logistic Regression setup. :contentReference[oaicite:12]{index=12}
...
- May over-rely on swear words or isolated tokens :contentReference[oaicite:13]{index=13}
...
- The method depends heavily on LIME stability :contentReference[oaicite:14]{index=14}
